...
import { ArrowLeft, Award, Briefcase, GraduationCap, User } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    if (currentJob) {
      ...
      suggestStudentProfiles({
        ...
        .then(suggestions => {
          const fullStudentDetails = suggestions.map(suggestion => {
            const originalStudent = students.find(s => s.id === suggestion.studentId);
    ...
    <div className="flex flex-col gap-6">
       <div>
        ...
        <p className="text-muted-foreground">
          Top student matches for your <span className="font-semibold text-primary">{job.title}</span> role.
        </p>
      </div>
      ...
      {matchedStudents.length > 0 ? (
            ...
            <Card key={student.id} className="flex flex-col">
              <CardHeader className="flex-row items-center gap-4">
                ...
                <div>
                    ...
                    <CardDescription className="flex flex-col gap-1 mt-1">
                        <span className="flex items-center gap-2"><GraduationCap className="h-4 w-4"/> {student.course}</span>
                        <span className="flex items-center gap-2"><User className="h-4 w-4"/> Year {student.year}</span>
                    </CardDescription>
                </div>
              </CardHeader>
              <CardContent className="flex-grow space-y-4">
                ...
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">{student.reasoning}</p>
                </div>
              </CardContent>
        ...
        <Card className="text-center p-8">
            ...
            <CardDescription className="mt-2">
                Our AI couldn't find any suitable candidates at the moment. Try adjusting your job description.
            </CardDescription>
        </Card>
      )}
    </div>
